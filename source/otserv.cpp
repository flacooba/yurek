...
//	LOG_MESSAGE("main", EVENT, 1, "Starting server");
	...
	std::cout << "[done]" << std::endl;
#ifdef TBB_IMBUING
	std::cout << ":: Loading imbuements.xml...          ";
	if(!g_imbuements.loadFromXml(g_config.getGlobalString("datadir")))
	{
		ErrorMessage("Could not load imbuements.xml!");
		return -1;
	}
	std::cout << "[done]" << std::endl;
/*
	ImbuementsMap::iterator it;
	for(it = g_imbuements.imbuements.begin(); it != g_imbuements.imbuements.end(); it++) {
        std::vector<Imbuement*> vec = it->second;
        for(std::vector<Imbuement*>::iterator it2 = vec.begin(); it2 != vec.end(); it2++) {
            std::cout << "Imbu name: " << (*it2)->name << ", id: " << (*it2)->id << ", level: " << (*it2)->level->id << ", value: " << (*it2)->value << std::endl;
        }
	}
*/
#endif // TBB_IMBUING
