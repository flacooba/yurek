			...
			for(tlIt = poslist.begin(); tlIt != poslist.end(); ++tlIt) {
				...
				if(creatureStateVec.empty()) { //no targets
					...
					for(CreatureStateVec::const_iterator csIt = creatureStateVec.begin(); csIt != creatureStateVec.end(); ++csIt) {
						...
						#ifdef TBB_IMBUING
						if(spectator == creature && creatureState.damage > 0) {
                            totalDmg += creatureState.damage;
                            totalCreatures++;
						}
						#endif // TBB_IMBUING
...
#ifdef TBB_IMBUING
    //simple mana leech
	if(totalDmg > 0 && creature) {
        Player* player = dynamic_cast<Player*>(creature);
        if(player && player->items[SLOT_HEAD] && player->items[SLOT_HEAD]->getAttributes()) {
            Imbuement* imbu = player->items[SLOT_HEAD]->getAttributes()->getImbuement(IMBUEMENT_MANA_LEECH);
            if(imbu) {
                int64_t manaLeech = totalDmg/totalCreatures;
                manaLeech *= std::pow((float)totalCreatures, 0.15f);
                manaLeech = (int)((float)manaLeech * ((float)imbu->value * 0.01f));
                std::cout << "Mana leeched: " << manaLeech << std::endl;
                std::stringstream manaLeechedStr;
				manaLeechedStr << manaLeech;
                player->mana += manaLeech;
                player->sendAnimatedText(player->pos, 16, manaLeechedStr.str());
...
#endif //YUR_INVISIBLE
#ifdef TBB_IMBUING
            for(int i = 1; i <= SLOT_AMMO; i++) {
                Item* checkItem = player->items[i];
                if(checkItem) {
                    ItemAttributes* attrs = checkItem->getAttributes();
                    if(attrs && attrs->imbuements) {
                        for(int j = 0; j < *attrs->getImbuementSlots(); j++) {
                            Imbuement* imbu = attrs->imbuements[j];
                            if(imbu) {
                                imbu->duration -= thinkTicks;

                                if(imbu->duration <= 0) {
                                    attrs->imbuements[j] = NULL;
                                    delete imbu;
                                }
                            }
                        }
