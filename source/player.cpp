...
#ifdef TBB_IMBUING
void Player::sendTextWindow(uint16_t itemId, std::string text, std::string writer, const uint16_t maxlen, const bool canWrite) {
	client->sendTextWindow(itemId, text, writer, maxlen, canWrite);
}
#endif // TBB_IMBUING
...
#ifdef TBB_IMBUING
void Player::setTempValue(uint32_t id, int32_t value) {
    tempValues[id] = value;
}
int32_t Player::getTempValue(uint32_t id) {
	TempValuesMap::const_iterator it = tempValues.find(id);
	return (it != tempValues.end())? it->second : -1;
}
void Player::sendChannelsDialog(std::list<ChannelInfo> channels) {
	client->sendChannelsDialog(channels);
}
#endif // TBB_IMBUING
#include "protocol76.h"
void Player::addCreature() {
        NetworkMessage msg;
        client->sendThingAppear(this);
