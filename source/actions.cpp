	...
	if (doc){
		...
		while (p)
		{
			...
			if (strcmp(str, "action") == 0){
				...
				#ifdef TBB_IMBUING
				char* scriptType = (char*)xmlGetProp(p,(xmlChar*)"type");
				if(scriptType) {
                    if(strcmp(scriptType, "openchannel") == 0) {
                        action = loadAction(p);
                        onOpenChannelActions.push_back(action);
                        action = NULL;
                    }
				} else
				#endif //TBB_IMBUING
...
#ifdef TBB_IMBUING
bool Actions::OpenChannel(Player* player, uint16_t id) {
    for(ActionVector::iterator it = onOpenChannelActions.begin(); it != onOpenChannelActions.end(); it++) {
        (*it)->executeOpenChannel(player, id);
    }
    return true;
}
...
#ifdef TBB_IMBUING
bool Action::executeOpenChannel(Player* player, uint16_t id) {
	//onOpenChannel(uidplayer, channelId)
	script->ClearMap();
	script->_player = player;
	PositionEx playerpos = player->pos;
	unsigned int cid = script->AddThingToMap((Thing*)player,playerpos);
	...
	lua_pushstring(luaState, "onOpenChannel");
	lua_gettable(luaState, LUA_GLOBALSINDEX);
...
#ifdef TBB_IMBUING
	//getPlayerTempValue(cid, id)
	lua_register(luaState, "getPlayerTempValue", ActionScript::luaActionGetPlayerTempValue);
	//setPlayerTempValue(cid, id, value)
	lua_register(luaState, "setPlayerTempValue", ActionScript::luaActionSetPlayerTempValue);
	//doPlayerSendChannelsDialog(cid, list)
	lua_register(luaState, "doPlayerSendChannelsDialog", ActionScript::luaActionDoPlayerSendChannelsDialog);
	//setItemAttribute(uid, id, value)
	lua_register(luaState, "setItemAttribute", ActionScript::luaActionSetItemAttribute);
	//getImbuementSlotsCount(uid)
	lua_register(luaState, "getImbuementSlotsCount", ActionScript::luaActionGetImbuementSlotsCount);
	//setImbuementSlotsCount(uid, count)
	lua_register(luaState, "setImbuementSlotsCount", ActionScript::luaActionSetImbuementSlotsCount);
	//getFreeImbuementSlotsCount(uid)
	lua_register(luaState, "getFreeImbuementSlotsCount", ActionScript::luaActionGetFreeImbuementSlotsCount);
	//getFirstFreeImbuementSlotId(uid)
	lua_register(luaState, "getFirstFreeImbuementSlotId", ActionScript::luaActionGetFirstFreeImbuementSlotId);
	//getThingPos(uid)
	lua_register(luaState, "getThingPos", ActionScript::luaActionGetThingPos);
	//getPlayerInventoryItem(uid)
	lua_register(luaState, "getPlayerInventoryItem", ActionScript::luaActionGetPlayerInventoryItem);
	//hasItemImbuement(uid)
	lua_register(luaState, "hasItemImbuement", ActionScript::luaActionHasItemImbuement);
	//sendPlayerTextWindow(cid, text[, maxlen[, canWrite]])
	lua_register(luaState, "sendPlayerTextWindow", ActionScript::luaActionSendPlayerTextWindow);
	//addItemImbuement(uid, id, level, time)
	lua_register(luaState, "addItemImbuement", ActionScript::luaActionAddItemImbuement);
#endif // TBB_IMBUING
...
int ActionScript::luaActionGetPlayerTempValue(lua_State *L)
{
	//getPlayerTempValue(cid,id)
	uint32_t tempValueId = (uint32_t)internalGetNumber(L);
	...
	if(tmp){
		Player *player = (Player*)(tmp->thing);
		int32_t value = player->getTempValue(tempValueId);
		lua_pushnumber(L, value);
		return 1;
	}
	else{
		lua_pushnumber(L, -1);
		std::cout << "GetPlayerTempValue: player not found" << std::endl;
		return 1;
	}
}

int ActionScript::luaActionSetPlayerTempValue(lua_State *L)
{
	//setPlayerTempValue(cid, id, value)
	uint32_t value = (uint32_t)internalGetNumber(L);
	uint32_t tempValueId = (uint32_t)internalGetNumber(L);
	uint32_t cid = (uint32_t)internalGetNumber(L);
	...
	if(tmp){
		Player *player = (Player*)(tmp->thing);
		player->setTempValue(tempValueId, value);
	}
	else{
		lua_pushnumber(L, -1);
		std::cout << "SetPlayerTempValue: player not found" << std::endl;
		return 1;
	}
	lua_pushnumber(L,0);
	return 1;
...
int ActionScript::luaActionDoPlayerSendChannelsDialog(lua_State *L)
{
	//doPlayerSendChannelsDialog(cid, channels)
	if(!lua_istable(L, -1)) {
		std::cout << "DoPlayerSendChannelsDialog: channels table not found" << std::endl;
		return 1;
	}
	std::list<ChannelInfo> channels;
    lua_pushnil(L);
    while (lua_next(L, -2) != 0) {
        uint32_t channelId = (uint32_t)lua_tonumber(L, -2);
        const char* text = lua_tostring(L, -1);
        channels.push_back(ChannelInfo(channelId, std::string(text)));
        lua_pop(L, 1);
    }
    lua_pop(L, 1);
	uint32_t cid = (uint32_t)internalGetNumber(L);
	...
	if(tmp){
		...
		lua_pushnumber(L, -1);
		std::cout << "luaActionDoPlayerSendChannelsDialog: player not found" << std::endl;
		return 1;
	}
	lua_pushnumber(L,0);
	return 1;
...
int ActionScript::luaActionSetItemAttribute(lua_State *L)
{
	//setItemAttribute(uid, type, value)
	std::string* textValue = NULL;
	uint32_t* numberValue = NULL;
	if(lua_isnumber(L, -1))
        numberValue = new uint32_t(internalGetNumber(L));
    else if(lua_isstring(L, -1))
        textValue = new std::string(internalGetString(L));
    else {
		std::cout << "luaActionSetItemAttribute: wrong attribute value" << std::endl;
    }

	uint32_t type = (uint32_t)internalGetNumber(L);
	...
	if(tmp){
        ...
        switch(type) {
            case ITEM_ATTRIBUTE_IMBU_SLOTS: {
                if(!numberValue) {
                    std::cout << "luaActionSetItemAttribute: wrong attribute value for type " << type << std::endl;
                    break;
                }
                attributes->setImbuementSlots(*numberValue);
                break;
            }
            case ITEM_ATTRIBUTE_IMBU_ID: {
                ...
                size_t pos = 0;
                int32_t imbuSlot = atoi((*textValue).substr(pos, (*textValue).find(',', pos)).c_str()) - 1;
                pos = (*textValue).find(',', pos)+1;
                int32_t id = atoi((*textValue).substr(pos, (*textValue).find(',', pos)).c_str());
                pos = (*textValue).find(',', pos)+1;
                int32_t level = atoi((*textValue).substr(pos, (*textValue).find(',', pos)).c_str());
                pos = (*textValue).find(',', pos)+1;
                int32_t duration = atoi((*textValue).substr(pos).c_str());

                if(!attributes->getImbuementSlots()) {
                    attributes->setImbuementSlots(1);
                }
                uint8_t freeSlot = attributes->getFirstFreeImbuementSlot();
                if(freeSlot == 255) {
                    ret = 2;
                }
                else {
                    extern Imbuements g_imbuements;
                    Imbuement* imbuement = g_imbuements.createNewImbuement(id, level - 1);
                    if(imbuement) {
                        imbuement->duration = duration;
                        attributes->imbuements[freeSlot] = imbuement;
                    }
                    else {
                        ret = 3;
                    }
            ...
            default: {
                std::cout << "luaActionSetItemAttribute: wrong attribute type " << type << std::endl;
                break;
            }
        }
		...
		lua_pushnumber(L, -1);
		std::cout << "luaActionSetItemAttribute: Item not found" << std::endl;
	}
...
int ActionScript::luaActionGetFreeImbuementSlotsCount(lua_State *L)
{
	//getFreeImbuementSlotsCount(uid)
	unsigned int uid = (unsigned int)internalGetNumber(L);
	...
	if(tmp){
		...
		lua_pushnumber(L, 0);
		std::cout << "luaActionGetFreeImbuementSlotsCount: Item not found" << std::endl;
		return 1;
	}

	ItemAttributes* attrs = tmpitem->getAttributes();
	int32_t freeSlots = 0;
	if(attrs && attrs->getImbuementSlots()) {
        int32_t count = 0;
        if(attrs->imbuements) {
            for(int i = 0; i < *attrs->getImbuementSlots(); i++) {
                if(attrs->imbuements[i] == NULL)
                    count++;
            }
        }
        else {
            count = *attrs->getImbuementSlots();
        }
...
int ActionScript::luaActionGetFirstFreeImbuementSlotId(lua_State *L)
{
	//getFirstFreeImbuementSlotId(uid)
	unsigned int uid = (unsigned int)internalGetNumber(L);
	...
	if(tmp){
		...
		lua_pushnumber(L, 0);
		std::cout << "luaActionGetFirstFreeImbuementSlotId: Item not found" << std::endl;
		return 1;
	}
    ...
    int32_t slotId = 255;
	if(attrs && attrs->getImbuementSlots()) {
        slotId = attrs->getFirstFreeImbuementSlot();
        if(slotId < 255)
            slotId++;
	}
...
int ActionScript::luaActionGetImbuementSlotsCount(lua_State *L)
{
	//getImbuementSlotsCount(uid)
	unsigned int uid = (unsigned int)internalGetNumber(L);
	...
	if(tmp){
		...
		lua_pushnumber(L, 0);
		std::cout << "luaActionGetImbuementSlotsCount: Item not found" << std::endl;
		return 1;
	}

	ItemAttributes* attrs = tmpitem->getAttributes();
	if(attrs && attrs->getImbuementSlots())
        lua_pushnumber(L, *attrs->getImbuementSlots());
    else
        lua_pushnumber(L, 0);
...
int ActionScript::luaActionSetImbuementSlotsCount(lua_State *L)
{
	//setImbuementSlotsCount(uid, count)
	unsigned int count = (unsigned int)internalGetNumber(L);
	unsigned int uid = (unsigned int)internalGetNumber(L);
	...
	if(tmp){
		...
		lua_pushnumber(L, -1);
		std::cout << "luaActionSetImbuementSlotsCount: Item not found" << std::endl;
		return 1;
	}
	...
	if(!attrs) {
        ...
        tmpitem->setAttributes(attrs);
	}
	attrs->setImbuementSlots(count);

    lua_pushnumber(L, 0);
	return 1;
}

int ActionScript::luaActionGetThingPos(lua_State *L)
{
	//getThingPos(uid)
	unsigned int uid = (unsigned int)internalGetNumber(L);
	...
	ActionScript::internalAddPositionEx(L, tmp->pos);

	return 1;
}

int ActionScript::luaActionHasItemImbuement(lua_State *L)
{
	//hasItemImbuement(uid, imbuId)
	unsigned int imbuId = (unsigned int)internalGetNumber(L);
	unsigned int uid = (unsigned int)internalGetNumber(L);
	...
	Item* item = (Item*)tmp->thing;
	ItemAttributes* attr = item->getAttributes();
	int32_t result = 0;
	if(attr) {
        if(attr->getImbuement(imbuId))
            result = 1;
	}
...
int ActionScript::luaActionAddItemImbuement(lua_State *L)
{
	//addItemImbuement(uid, id, level, time)
	unsigned long imbuTime = (unsigned long)internalGetNumber(L);
	unsigned int imbuLevel = (unsigned int)internalGetNumber(L);
	unsigned int imbuId = (unsigned int)internalGetNumber(L);
	unsigned int uid = (unsigned int)internalGetNumber(L);
	...
	int32_t result = 0;
	if(attr) {
        uint8_t freeSlot = attr->getFirstFreeImbuementSlot();
        if(freeSlot < 255) {
            if(attr->addNewImbuement(freeSlot, imbuId, imbuLevel, imbuTime))
                result = 1;
        }
	}
...
int ActionScript::luaActionGetPlayerInventoryItem(lua_State *L)
{
	//getPlayerInventoryItem(cid, slotId)
	uint32_t slotId = (uint32_t)internalGetNumber(L);
	uint32_t cid = (uint32_t)internalGetNumber(L);

	if(slotId < SLOT_HEAD || slotId > SLOT_AMMO) {
		lua_pushnumber(L, -1);
		std::cout << "luaActionGetPlayerInventoryItem: wrong slot id" << std::endl;
		return 1;
	}
	...
	if(tmp){
		Player *player = (Player*)(tmp->thing);
        Item* slotItem = player->items[slotId];
        if(slotItem) {
            PositionEx posEx = PositionEx(slotItem->pos);
            uint32_t uid = action->AddThingToMap(slotItem, posEx);
            internalAddThing(L, slotItem, uid);
            return 1;
        }
	}
	else{
		std::cout << "luaActionGetPlayerInventoryItem: player not found" << std::endl;
	}
	lua_pushnumber(L,-1);
	return 1;
}

int ActionScript::luaActionSendPlayerTextWindow(lua_State *L){
	//sendPlayerTextWindow(cid, itemId, text, maxlen, canWrite)
	bool canWrite = false;
	uint16_t maxlen = 512;
	...
	if(lua_isboolean(L, -1)) {
        lua_pop(L,1);
        canWrite = lua_toboolean(L, 0) != 0;
	}
	if(lua_isnumber(L, -1)) {
        maxlen = (uint16_t) internalGetNumber(L);
	}
	if(lua_isstring(L, -1)) {
        text = std::string(internalGetString(L));
	}
	if(lua_isstring(L, -1)) {
       writer = text;
       text = std::string(internalGetString(L));
	...
	if(tmp) {
        Player* player = (Player*)(tmp->thing);
        player->sendTextWindow(itemId, text, writer, maxlen, canWrite);
	}
	else {
		std::cout << "luaActionSendPlayerTextWindow: player not found" << std::endl;
	}
