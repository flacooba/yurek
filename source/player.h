...
struct ChannelInfo {
    ChannelInfo() {}
    ChannelInfo(uint16_t _id, std::string _name) : id(_id), name(_name) {}
    uint16_t id;
    std::string name;
};
#endif //TBB_IMBUING
...
#ifdef TBB_IMBUING
typedef std::map<uint32_t, int32_t> TempValuesMap;
#endif //TBB_IMBUING
...
#ifdef TBB_IMBUING
	void sendTextWindow(uint16_t itemId, std::string text, std::string writer = "unknown", const uint16_t maxlen = 512, const bool canWrite = false);
#endif // TBB_IMBUING
...
#ifdef TBB_IMBUING
   int32_t getTempValue(uint32_t id);
   void setTempValue(uint32_t id, int32_t value);
   void sendChannelsDialog(std::list<ChannelInfo> channels);
#endif //TBB_IMBUING
    void addCreature();
