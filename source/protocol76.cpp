...
void Protocol76::parseOpenChannel(NetworkMessage &msg){
    ...
    #ifdef TBB_IMBUING
    extern Actions actions;
    actions.OpenChannel(player, channelId);
    #endif // TBB_IMBUING
}
...
#ifdef TBB_IMBUING
void Protocol76::sendChannelsDialog(std::list<ChannelInfo> channels) {
    if(channels.empty())
        return;
	...
	newmsg.AddByte(0xAB);
	newmsg.AddByte((unsigned char)channels.size()); //how many

	while(channels.size())
	{
		ChannelInfo& channelInfo = channels.front();
		newmsg.AddU16(channelInfo.id);
		newmsg.AddString(channelInfo.name);
		channels.pop_front();
	}
	WriteBuffer(newmsg);
}
...
#ifdef TBB_IMBUING
void Protocol76::sendTextWindow(uint16_t itemId, std::string& text, std::string& writer, const uint16_t maxlen, const bool canWrite) {
	NetworkMessage msg;
	msg.AddByte(0x96);
	windowTextID++;
	msg.AddU32(windowTextID);
	msg.AddU16(Item::items[itemId].clientId);
    readItem = NULL;
	if(canWrite){
		msg.AddU16(maxlen);
		msg.AddString(text);
